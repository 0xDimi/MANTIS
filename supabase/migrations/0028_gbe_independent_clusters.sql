...
-- Event-aware trade execution overload.
-- The existing per-market RPC remains in place for standalone markets and house
-- ops. Public app trades for event children call this overload so event exposure
-- is checked under a parent-event row lock before the atomic market trade runs.
create or replace function public.execute_alpha_trade_with_event_guard(
  p_user_id uuid,
  p_market_id uuid,
  p_side trade_side,
  ...
  p_max_user_event_exposure numeric(14,2) default null
)
returns table (
  trade_id uuid,
...
declare
  ...
  v_event_open_exposure numeric(14,2);
begin
  if p_max_user_event_exposure is not null and p_max_user_event_exposure <= 0 then
    raise exception 'max_user_event_exposure must be > 0';
  end if;
  ...
  if (
    p_action = 'buy'
    and p_max_user_event_exposure is not null
    and v_market.is_event_child = true
    and v_market.event_id is not null
  ) then
    perform 1
    from market_events
    ...
    select coalesce(sum(coalesce(p.yes_cost_basis, 0) + coalesce(p.no_cost_basis, 0)), 0)
      into v_event_open_exposure
    from positions p
    join markets m on m.id = p.market_id
    where p.user_id = p_user_id
      and m.event_id = v_market.event_id
      and m.is_event_child = true;

    if v_event_open_exposure + p_total_amount > p_max_user_event_exposure then
      raise exception 'max user event exposure exceeded';
    end if;
  end if;

  return query
  select *
  from public.execute_alpha_trade(
    p_user_id,
    p_market_id,
    p_side,
    p_action,
...
revoke all on function public.execute_alpha_trade_with_event_guard(
  uuid,
  uuid,
  trade_side,
  trade_action,
...
grant execute on function public.execute_alpha_trade_with_event_guard(
  uuid,
  uuid,
  trade_side,
  trade_action,
