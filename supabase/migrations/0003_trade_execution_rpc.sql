-- 0003_trade_execution_rpc.sql
-- Week 3 foundation: atomic trade execution RPC for paper trading
...
begin
  ...
  if p_action = 'buy' then
    ...
    if p_side = 'yes' then
      update positions
      set
        yes_shares = yes_shares + p_share_delta,
        yes_cost_basis = yes_cost_basis + p_total_amount,
        updated_at = now()
      ...
      update positions
      set
        no_shares = no_shares + p_share_delta,
        no_cost_basis = no_cost_basis + p_total_amount,
        updated_at = now()
      where id = v_position.id
      returning * into v_position;
    end if;
    ...
    if p_side = 'yes' then
      ...
      set
        yes_shares = yes_shares - p_share_delta,
        yes_cost_basis = greatest(yes_cost_basis - v_cost_released, 0),
        realized_pnl = realized_pnl + v_realized_delta,
        updated_at = now()
      where id = v_position.id
      returning * into v_position;
      ...
      set
        no_shares = no_shares - p_share_delta,
        no_cost_basis = greatest(no_cost_basis - v_cost_released, 0),
        realized_pnl = realized_pnl + v_realized_delta,
        updated_at = now()
      where id = v_position.id
      returning * into v_position;
